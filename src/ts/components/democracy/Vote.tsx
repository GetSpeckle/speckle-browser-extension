...
import { ReferendumInfo, ChainProperties, Method } from '@polkadot/types'
...
interface IVoteState {
  idNumber: number
  referendum?: ReferendumInfo | null
  voteCount: number
  voteCountAye: number
  voteCountNay: number
  votedAye: BN
...
class Vote extends React.Component<IVoteProps, IVoteState> {
  ...
  updateVote = () => {
    if (this.props.apiContext.apiReady) {
      ...
      this.setState({ ...this.state, referendum: null })
    }
  }
  ...
  private doUpdate = () => {
    this.api.query.democracy.referendumInfoOf(this.props.match.params['proposalId'], (referendum) => {
      if (referendum !== this.state.referendum) {
        ...
        const referendumInfo: ReferendumInfo = referendum.unwrap()
  ...
  render () {
    ...
    return this.state.referendum !== undefined ? this.renderProposal() : this.renderPlaceHolder()
  }
